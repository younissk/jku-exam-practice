...
import { Button, Container, Text, List, ListItem, Stack, Progress } from '@mantine/core';
import { MultipleChoiceQuestion } from "../../../data/interfaces/Test";
import TestEndScreen from './TestEndScreen';

const TestSimulation = ({ questions }: { questions: MultipleChoiceQuestion[] }) => {
    const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
    const [selectedAnswers, setSelectedAnswers] = useState<{ [key: string]: string[] }>({});
    const [feedback, setFeedback] = useState<{ [key: string]: { option: string; correct: boolean; missed: boolean; }[] }>({});
    ...
    if (showEndScreen) {
        ...
        return <TestEndScreen totalQuestions={totalQuestions} />;
    }

    return (
        <Container style={{ width: "100%" }}>
            <Progress value={progress} size="md" mb="lg" />
            <Text>{totalAnswered}/{questions.length}</Text>
            <Stack align="center">
                ...
                <List type="ordered" spacing="sm">
                    {currentQuestion.options.map((option) => {
                        ...
                        const feedbackOption = feedback[currentQuestion.id]?.find(ans => ans.option === option);
                ...
                {showCorrectAnswers ? (
                    <>
                        <Text mt="md">Correct answers: {currentQuestion.correctOptions.join(', ')}</Text>
                        <Button onClick={handleNextQuestion} mt="md">Next Question</Button>
                    </>
                ) : (
                    <Button onClick={handleCheckAnswer} mt="md">Check Answer</Button>
                )}
            </Stack>
        </Container>
    );
